...
- Path: `packages/apps/workflow-nodes/src/services/workflow/executor/runtime`
...
- Functions (all):
  - `resolvePayloadCredential` (L162-L162, arrow)
- Exports:
  - `executeWorkflowManagementBackendRequest` (L271)
  - `getWorkflowExecutionRequestContext` (L271)
- Key snippets and use-case mapping:
  - `L162-L162`: Implements `resolvePayloadCredential` for this module use case.
### `backend-node-handler.ts`
- Purpose: Defines module behavior owned by this usage folder.
- Owning use cases: Runtime and application flows that import this file through package boundaries.
- Input shape: Typed arguments and imported contracts declared in this file signatures.
- Output shape: Typed return values, thrown errors, and exported contracts declared in this file.
...
- Exports:
  - None
- Key snippets and use-case mapping:
  - `L1-L63`: File-level constants/types behavior.
### `runtime.ts`
- Purpose: Defines module behavior owned by this usage folder.
- Owning use cases: Runtime and application flows that import this file through package boundaries.
- Input shape: Typed arguments and imported contracts declared in this file signatures.
...
- Logic gates summary:
  ...
  - MCP or GraphQL proxy boundaries must avoid duplicated domain validation.
- Functions (all):
  - `compileWorkflowCypherBridge` (L22-L22, arrow)
  - `hasRelativeWorkerImports` (L62-L62, arrow)
  - `runtimeEnvironmentForModel` (L64-L64, arrow)
- Exports:
  - None
- Key snippets and use-case mapping:
  - `L22-L22`: Implements `compileWorkflowCypherBridge` for this module use case.
  - `L62-L62`: Implements `hasRelativeWorkerImports` for this module use case.
  - `L64-L64`: Implements `runtimeEnvironmentForModel` for this module use case.
