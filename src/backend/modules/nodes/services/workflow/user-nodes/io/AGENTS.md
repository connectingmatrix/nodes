...
- Path: `packages/apps/workflow-nodes/src/services/workflow/user-nodes/io`
...
- Functions (all):
  - `isSafeSourcePath` (L16-L16, arrow)
  - `validateNodeSchema` (L37-L37, arrow)
  - `normalizeRow` (L52-L52, arrow)
- Exports:
  - `validateNodeSchema` (L37)
  - `normalizeRow` (L52)
- Key snippets and use-case mapping:
  - `L16-L16`: Implements `isSafeSourcePath` for this module use case.
  - `L37-L37`: Implements `validateNodeSchema` for this module use case.
  - `L52-L52`: Implements `normalizeRow` for this module use case.
### `package-export.ts`
- Purpose: Defines module behavior owned by this usage folder.
- Owning use cases: Runtime and application flows that import this file through package boundaries.
- Input shape: Typed arguments and imported contracts declared in this file signatures.
...
- Logic gates summary:
  ...
  - MCP or GraphQL proxy boundaries must avoid duplicated domain validation.
- Functions (all):
  - `sortKeys` (L4-L4, arrow)
- Exports:
  - None
- Key snippets and use-case mapping:
  - `L4-L4`: Implements `sortKeys` for this module use case.
### `package-parse.ts`
- Purpose: Defines module behavior owned by this usage folder.
- Owning use cases: Runtime and application flows that import this file through package boundaries.
- Input shape: Typed arguments and imported contracts declared in this file signatures.
...
- Logic gates summary:
  ...
  - MCP or GraphQL proxy boundaries must avoid duplicated domain validation.
- Functions (all):
  - `safePath` (L11-L11, arrow)
- Exports:
  - None
- Key snippets and use-case mapping:
  - `L11-L11`: Implements `safePath` for this module use case.
