...
import { Nodes, type NodeSourceFile } from './index.js';

test('@connectingmatrix/nodes crud, debug AI, executor, and .node packages work', async () => {
  const ctx = { userId: 'u1' };
  Nodes.useFileModule({
    createSourceArchiveAdapter: () => ({
      async createArchive(entries: NodeSourceFile[]) { return new TextEncoder().encode(JSON.stringify(entries)); },
      async extractArchive(archive: Uint8Array | string) {
        const text = archive instanceof Uint8Array ? new TextDecoder().decode(archive) : archive;
        return JSON.parse(text) as NodeSourceFile[];
      }
    })
  });
